...
in_port_t src_port = 0;              /**< local incomming port */
in_port_t dest_port = 0;             /**< destination port - where to send */
		...
		if (FD_ISSET(udt, &readfds)) {	// New incomming packet
            ...
            // Check whether has at least header and checksum passes
   			if (n >= DATA_OFFSET && testCheckSum(recv_packet, n)) {
                if (hasFlags(recv_packet,ACK)) {          // Ack recieved
                    removePacket(&window, seqNumber(recv_packet));
                    // Recieved last ACK packet, exiting
                    if (feof(stdin) && isEmpty(&window)) break;
	    ...
	    if ((setSTDIN = isAvailable(&window))) {
    		if (FD_ISSET(STDIN_FILENO, &readfds)) { // we have read a new line to send
                if (fgets(input_line, MAXLINE, stdin) != NULL ) {
                    stopTimer();
        			packet = makeDataPacket(input_line);    // Creating packet
        			sendPacket(packet);                     // Sending packet
        			storePacket(&window, cnt_seq, packet);  // Storing packet to window
        			cnt_seq++;                              // Incrementing sequence number
        			startTimer();
        		}
    		} else {
